...
from api.services.verified_consumer import VerifiedEventConsumer
...
async def _verified_consumer_worker():
    consumer = VerifiedEventConsumer()
    app.state.verified_consumer_instance = consumer
    try:
        await asyncio.to_thread(consumer.run_loop)
    except asyncio.CancelledError:
        consumer.stop()
        raise
    except Exception as exc:
        import logging
        logging.getLogger(__name__).warning("Verified consumer worker failed: %s", exc)


@app.on_event("startup")
async def start_background_workers():
    app.state.verification_outbox_task = asyncio.create_task(_verification_outbox_worker())
    app.state.verified_consumer_task = asyncio.create_task(_verified_consumer_worker())


@app.on_event("shutdown")
async def stop_background_workers():
    consumer = getattr(app.state, "verified_consumer_instance", None)
    if consumer:
        consumer.stop()

    for task_name in ("verification_outbox_task", "verified_consumer_task"):
...
app.include_router(health_router)  # Root /health alias for load balancers and orchestrators
